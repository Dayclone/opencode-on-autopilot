...
export function startScheduledSession(callback: () => void): void {
    ...
    try {
        ...
        debugLog(`🕒 Scheduling OpenCode session start for ${config.session.scheduledStartTime} (${formatRelativeTime(msUntilStart)} from now)`);
        
        scheduledTimer = setTimeout(() => {
            debugLog('🚀 Executing scheduled OpenCode session start');
            vscode.window.showInformationMessage('🚀 Starting scheduled OpenCode session...');
            callback();
        ...
        vscode.window.showInformationMessage(`OpenCode session scheduled to start at ${timeString}`);
