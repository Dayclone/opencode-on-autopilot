...
export function isCurrentUsageLimit(output: string): boolean {
    try {
        const usageLimitPattern = /(OpenCode\s+)?usage\s+limit\s+reached.*?reset\s+at\s+(\d{1,2}[:\d]*(?:\s*[APM]{2})?(?:\s*\([^)]+\))?)/gi;
...
export function handleUsageLimit(output: string, message: MessageItem): void {
    ...
    vscode.window.showWarningMessage(`OpenCode usage limit reached. Added "continue" message to queue. Will automatically resume processing at ${resetTime} (${waitMinutes} minutes)`);
