...
export function showInstallationInstructions(results: DependencyCheckResults): void {
    const { opencode, python, wrapper, ngrok } = results;
    
    let instructions = '# Opencode on Autopilot - Dependency Installation Guide\n\n';
    
    // Show status for each dependency
    if (opencode.available) {
        instructions += `${DebugEmojis.SUCCESS} OpenCode CLI: ${opencode.version}\n\n`;
    } else {
        instructions += `${DebugEmojis.ERROR} OpenCode CLI: Missing\n`;
        instructions += opencode.installInstructions + '\n\n';
    }
...
function analyzeDependencyStatus(results: DependencyCheckResults): DependencyStatus {
    ...
    if (!opencode.available) {
        issues.push(`${DebugEmojis.ERROR} OpenCode CLI: ${opencode.error}`);
    ...
    if (allCriticalReady) {
        successMessages.push(
            `${DebugEmojis.SUCCESS} All dependencies ready! OpenCode: ${opencode.version}, Python: ${python.version}, ngrok: ${ngrok.version || 'not available'}`
        );
    }
