...
import { 
    ...
    stopProcessingQueue 
} from '../../../opencode/communication';
import { startOpenCodeSession, handleOpenCodeKeypress } from '../../../opencode/session';
import { 
    messageQueue, 
    isRunning, 
    opencodeOutputBuffer, 
    processingQueue,
    sessionReady,
    currentMessage,
    setProcessingQueue,
...
export class APIRoutes {
    ...
    setupRoutes(app: Application): void {
        ...
        app.post('/api/control/start', async (req: Request, res: Response) => {
            try {
                if (!sessionReady) {
                    await startOpenCodeSession(true);
                }
                await startProcessingQueue(true);
                this.notifyCallback?.(MESSAGE_TYPES.STATUS_UPDATE);
                res.json({ success: true });
            } catch (error) {
                res.status(500).json({ error: 'Failed to start OpenCode' });
            }
        });
        ...
        app.post('/api/control/stop', async (req: Request, res: Response) => {
            try {
                ...
                res.status(500).json({ error: 'Failed to stop OpenCode' });
            }
        });
        ...
        app.post('/api/control/interrupt', (req: Request, res: Response) => {
            try {
                handleOpenCodeKeypress('escape');
                res.json({ success: true });
            } catch (error) {
                console.error('Error interrupting OpenCode:', error);
                res.status(500).json({ error: 'Failed to interrupt OpenCode' });
            }
        });
