...
import { appendToTerminal, appendToOpenCodeOutput, appendOpenCodeMessage, clearOpenCodeOutputUI } from '../ui/output-handlers.js';
...
export function setupMessageHandler() {
    window.addEventListener('message', event => {
        ...
        // Debug logging for output messages
        if (message.command === 'opencodeOutput' || message.command === 'opencodeOutputAppend') {
            console.log('📨 Received message:', message.command, 'length:', message.output?.length);
        }
        ...
        case 'terminalOutput':
            ...
            break;
        case 'opencodeOutput':
            appendToOpenCodeOutput(message.output);
            break;
        case 'opencodeOutputAppend':
            console.log('📥 Calling appendOpenCodeMessage with:', message.output?.substring(0, 100));
            appendOpenCodeMessage(message.output, message.timestamp);
            break;
        ...
        case 'webServerStatusUpdate':
            ...
            break;
        case 'clearOpenCodeOutput':
            clearOpenCodeOutputUI();
            break;
