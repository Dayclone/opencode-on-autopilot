// Global state management for the Opencode on Autopilot extension
...
let messageQueue = [];
let sessionState = {
    isSessionRunning: false,  // OpenCode session/process is active
...
// OpenCode output state
let opencodeContent = '';
let lastRenderedContent = '';
let pendingOpenCodeOutput = null;
let opencodeRenderTimer = null;
let lastOpenCodeRenderTime = 0;
let lastParsedContent = '';
let lastParsedHtml = '';
let lastContentHash = '';
// Dynamic throttling: slower in development mode to reduce overhead
function getOpenCodeRenderThrottleMs() {
...
const ansiColors = {
    ...
    // More colors including common OpenCode colors
...
export {
    ...
    opencodeContent,
    lastRenderedContent,
    pendingOpenCodeOutput,
    opencodeRenderTimer,
    lastOpenCodeRenderTime,
    lastParsedContent,
    lastParsedHtml,
    lastContentHash,
    CLAUDE_RENDER_THROTTLE_MS,
    getOpenCodeRenderThrottleMs,
...
// OpenCode output state management
export function setOpenCodeContent(content) {
...
export function setPendingOpenCodeOutput(output) {
    pendingOpenCodeOutput = output;
}

export function setOpenCodeRenderTimer(timer) {
    opencodeRenderTimer = timer;
}

export function setLastOpenCodeRenderTime(time) {
    lastOpenCodeRenderTime = time;
}
...
// Getters for OpenCode output state
export function getOpenCodeContent() {
...
export function getPendingOpenCodeOutput() {
    return pendingOpenCodeOutput;
}

export function getOpenCodeRenderTimer() {
    return opencodeRenderTimer;
}

export function getLastOpenCodeRenderTime() {
    return lastOpenCodeRenderTime;
}
...
export function resetOpenCodeOutputState() {
    opencodeContent = '';
    lastRenderedContent = '';
    lastParsedContent = '';
    lastParsedHtml = '';
    lastContentHash = '';
    pendingOpenCodeOutput = null;
    if (opencodeRenderTimer) {
        clearTimeout(opencodeRenderTimer);
        opencodeRenderTimer = null;
    }
    lastOpenCodeRenderTime = 0;
}
