...
export class MobileInterface {
    constructor() {
        ...
        this.opencodeContent = '';
        this.lastRenderedContent = '';
        this.lastParsedContent = '';
        this.lastParsedHtml = '';
        
        // Throttling mechanism (exactly like main extension)
        this.pendingOpenCodeOutput = null;
        this.opencodeRenderTimer = null;
        this.lastOpenCodeRenderTime = 0;
        ...
        try {
            ...
            this.ws.onopen = () => {
                ...
                if (!this.hasShownConnectedToast) {
                    this.showToast('Connected to Opencode on Autopilot', TOAST_TYPE.SUCCESS);
    ...
    updateControlButtons(status) {
        ...
        // Interrupt should be enabled when OpenCode process is running, even if not fully ready
    ...
    async handleControlAction(action) {
        ...
        try {
            ...
            await response.json();
            this.showToast(`Successfully ${action}ed Opencode on Autopilot`, TOAST_TYPE.SUCCESS);
        } catch (error) { // eslint-disable-line no-unused-vars
            this.showToast(`Failed to ${action} Opencode on Autopilot`, TOAST_TYPE.ERROR);
        } finally {
            this.hideLoading();
        }
    }
    ...
    // Mobile version of appendToOpenCodeOutput (exactly like main extension)
    appendOutput(output) {
        try {
            // Store the latest output
            this.pendingOpenCodeOutput = output;
            
            // Check if we need to throttle
            const now = Date.now();
            const timeSinceLastRender = now - this.lastOpenCodeRenderTime;
            
            if (timeSinceLastRender >= this.CLAUDE_RENDER_THROTTLE_MS) {
                // Enough time has passed, render immediately
                this.renderOpenCodeOutput();
            } else {
                // Schedule a delayed render if not already scheduled
                if (!this.opencodeRenderTimer) {
                    const delay = this.CLAUDE_RENDER_THROTTLE_MS - timeSinceLastRender;
                    this.opencodeRenderTimer = setTimeout(() => {
                        this.renderOpenCodeOutput();
                    }, delay);
                }
            }
        } catch (error) { // eslint-disable-line no-unused-vars
            // Error appending to OpenCode output
        }
    }

    renderOpenCodeOutput() {
        if (!this.pendingOpenCodeOutput) {
            return;
        }
        
        const output = this.pendingOpenCodeOutput;
        this.pendingOpenCodeOutput = null;
        this.lastOpenCodeRenderTime = Date.now();
        
        // Clear the timer
        if (this.opencodeRenderTimer) {
            clearTimeout(this.opencodeRenderTimer);
            this.opencodeRenderTimer = null;
        }
        
        // Now perform the actual rendering
        this.performOpenCodeRender(output);
    }
    ...
    setOutput(output) {
        if (output) {
            ...
            const outputStream = document.getElementById('opencode-output');
            outputStream.innerHTML = '<div class="output-line" data-type="system">📱 Mobile interface ready...</div>';
        }
    }

    // Exact same logic as main extension's performOpenCodeRender
    performOpenCodeRender(output) {
        try {
            const opencodeOutput = document.getElementById('opencode-output');
            ...
            if (opencodeOutput.innerHTML.includes('Mobile interface ready')) {
            ...
            if (output.includes('\x1b[2J') || output.includes('\x1b[3J') || output.includes('\x1b[H')) {
                ...
                const contentToRender = this.opencodeContent.replace(/\x1b\[[2-3]J/g, '').replace(/\x1b\[H/g, '');
                ...
                opencodeOutput.appendChild(outputElement);
            } else {
                ...
                if (output !== this.lastRenderedContent) {
                    ...
                    if (output === this.lastParsedContent && this.lastParsedHtml) {
                        ...
                        htmlOutput = parseAnsiToHtml(this.opencodeContent);
                        this.lastParsedContent = output;
                        this.lastParsedHtml = htmlOutput;
                    }
            ...
            // Error rendering OpenCode output
        }
    }
    ...
    // Keyboard shortcuts
        ...
        if (e.key === 'Escape') {
            ...
            if (addModal.style.display === 'flex' || editModal.style.display === 'flex') {
                ...
                // If no modals are open, send interrupt to OpenCode
                this.handleControlAction('interrupt');
            }
        }
    }
