...
import { opencodePanel, messageQueue, debugMode, sessionReady, processingQueue } from '../../core/state';
...
export function getWebviewContent(context: vscode.ExtensionContext, webview?: vscode.Webview): string {
    ...
    try {
        ...
        // Use the passed webview or fall back to opencodePanel
        const activeWebview = webview || opencodePanel?.webview;
        if (!activeWebview) {
        ...
        return `
            ...
            <html>
            <head>
                <title>Opencode on Autopilot</title>
                <style>
                    body { 
