...
export interface OpenCodeAutopilotConfig {
...
export const DEFAULT_CONFIG: OpenCodeAutopilotConfig = {
...
export function validateConfig(config: Partial<OpenCodeAutopilotConfig>): ConfigValidationError[] {
    ...
    if (config.session) {
        ...
        if (s.autoStart === true && s.scheduledStartTime !== undefined && s.scheduledStartTime !== '') {
            addError('session.scheduledStartTime', 'both enabled', 'only one enabled', 'Cannot use both autoStart and scheduledStartTime - choose one or the other');
        }
    }
...
export function getValidatedConfig(): OpenCodeAutopilotConfig {
    ...
    const config: OpenCodeAutopilotConfig = {
        ...
        session: {
            ...
            freshSessionPerMessage: workspaceConfig.get('session.freshSessionPerMessage', DEFAULT_CONFIG.session.freshSessionPerMessage)
        },
    ...
    if (errors.length > 0) {
        ...
        vscode.window.showWarningMessage(
            `Opencode on Autopilot configuration has invalid values:\n${errorMessages}\n\nUsing default values for invalid settings.`,
            'Open Settings'
        ).then(selection => {
            if (selection === 'Open Settings') {
                vscode.commands.executeCommand('workbench.action.openSettings', 'opencodeOnAutopilot');
            }
        });
...
function getDefaultsForInvalidConfig(config: OpenCodeAutopilotConfig, errors: ConfigValidationError[]): OpenCodeAutopilotConfig {
...
export function resetConfigToDefaults(): void {
    ...
    Promise.all(resetPromises).then(() => {
        vscode.window.showInformationMessage('Opencode on Autopilot configuration reset to defaults');
...
export function showConfigValidationStatus(): void {
    ...
    if (errors.length === 0) {
        if (debugMode) {
            vscode.window.showInformationMessage('✅ Opencode on Autopilot configuration is valid');
        }
    } else {
...
export function watchConfigChanges(callback: (config: OpenCodeAutopilotConfig) => void): vscode.Disposable {
    return vscode.workspace.onDidChangeConfiguration(event => {
        if (event.affectsConfiguration('opencodeOnAutopilot')) {
            debugLog('🔧 Opencode on Autopilot configuration changed, revalidating...');
