...
import { opencodePanel, debugMode } from '../../core/state';
...
export function formatTerminalOutput(text: string, type: 'opencode' | 'debug' | 'error' | 'info' | 'success'): string {
    const now = new Date();
...
export function getHistoryStorageKey(): string {
    return `opencodeautopilot_history_${getWorkspacePath().replace(/[^a-zA-Z0-9]/g, '_')}`;
}

export function getPendingQueueStorageKey(): string {
    return `opencodeautopilot_pending_${getWorkspacePath().replace(/[^a-zA-Z0-9]/g, '_')}`;
}